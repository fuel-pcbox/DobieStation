...
namespace vu
{
    VectorInterface::VectorInterface(gs::GraphicsInterface* gif, vu::VectorUnit* vu, ee::INTC* intc, ee::DMAC* dmac, int id) :
    ...
    bool VectorInterface::check_vif_stall(uint32_t value)
    {
        ...
        if (command == 0)
        {
            ...
            if (vif_ibit_detected)
            {
                ...
                if (get_id())
                    intc->assert_IRQ((int)ee::Interrupt::VIF1);
                else
                    intc->assert_IRQ((int)ee::Interrupt::VIF0);
