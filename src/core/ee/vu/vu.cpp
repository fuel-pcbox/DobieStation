...
namespace vu
{
    ...
    VectorUnit::VectorUnit(int id, core::Emulator* e, ee::INTC* intc, ee::EmotionEngine* cpu, VectorUnit* other_vu) :
    ...
    void VectorUnit::set_GIF(gs::GraphicsInterface *gif)
    {
        this->gif = gif;
    }
    ...
    void VectorUnit::run()
    {
        ...
        while (running && cycles_to_run > 0)
        {
            ...
            vu::interpreter::interpret(*this, upper_instr, lower_instr);
            ...
            if (upper_instr & (1 << 27))
            {
                if (read_fbrst() & (1 << (3 + (get_id() * 8))))
                {
                    if (!get_id())
                    {
                        intc->assert_IRQ((int)ee::Interrupt::VU0);
                    }
                    else
                    {
                        intc->assert_IRQ((int)ee::Interrupt::VU1);
                    }
                    tbit_stop = true;
                    running = false;
                    finish_on = false;
    ...
    void VectorUnit::disasm_micromem()
    {
        ...
        vu::jit::set_current_program(crc, this);
    ...
    void VectorUnit::start_program(uint32_t addr, uint32_t cycle_delay)
    {
        ...
        if (is_dirty())
        {
            vu::jit::set_current_program(crc_microprogram(), this);
            clear_dirty();
        }
