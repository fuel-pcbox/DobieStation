...
namespace vu
{
    namespace disasm
    {
        ...
        {
            std::stringstream output;
            std::string bc_field = get_fsf(instr & 0x3);
        ...
        std::string upper_acc(const std::string op, uint32_t instr)
        {
            ...
            std::string field = "." + get_field(dest_field);
            output << op << field;
            output << " ACC, vf" << fs;
            output << ", vf" << ft;
            return output.str();
        ...
        {
            std::stringstream output;
            output << "loi 0x" << std::setfill('0') << std::setw(8) << std::hex << lower;
            return output.str();
        }
        ...
        std::string lower1_immi(const std::string op, uint32_t instr)
        {
            ...
            output << std::setfill('0') << std::setw(4) << std::hex << imm;
            return output.str();
        }
        ...
        std::string div(uint32_t instr)
        {
            ...
            uint32_t fs = (instr >> 11) & 0x1F;
            uint32_t ft = (instr >> 16) & 0x1F;
            std::string fsf = get_fsf((instr >> 21) & 0x3);
            std::string ftf = get_fsf((instr >> 23) & 0x3);
        ...
        std::string branch(const std::string op, uint32_t PC, uint32_t instr)
        {
            ...
            output << std::setfill('0') << std::setw(4) << std::hex << addr;
            return output.str();
        }
        ...
        std::string branch_zero(const std::string op, uint32_t PC, uint32_t instr)
        {
            ...
            uint32_t addr = PC + imm + 8;
            output << op << " vi" << reg << ", $" << std::setfill('0') << std::setw(4) << std::hex << addr;
            return output.str();
        }
        ...
        std::string fceq(uint32_t instr)
        {
            ...
            output << "fceq vi1, 0x";
            output << std::setfill('0') << std::setw(8) << std::hex << imm;
            return output.str();
        }
        ...
        std::string fcset(uint32_t instr)
        {
            std::stringstream output;
            uint32_t imm = instr & 0xFFFFFF;
            output << "fcset 0x" << std::setfill('0') << std::setw(8) << std::hex << imm;
            return output.str();
        }
        ...
        std::string b(uint32_t PC, uint32_t instr)
        {
            ...
            uint32_t addr = PC + imm + 8;
            output << "b $" << std::setfill('0') << std::setw(4) << std::hex << addr;
            return output.str();
        }
        ...
        std::string bal(uint32_t PC, uint32_t instr)
        {
            ...
            uint32_t addr = PC + imm + 8;
            output << "bal vi" << link_reg << ", $" << std::setfill('0') << std::setw(4) << std::hex << addr;
            return output.str();
        }
