...
namespace ee
{
    BIOS_HLE::BIOS_HLE(core::Emulator* e, gs::GraphicsSynthesizer* gs) : 
        e(e), gs(gs)
    {
    ...
    void BIOS_HLE::store(uint32_t& address, uint32_t value)
    {
    ...
    void BIOS_HLE::assemble_interrupt_handler()
    {
        ...
        store(addr, lui(MIPS_REG::at, 0x00030000));
        store(addr, ori(MIPS_REG::k0, MIPS_REG::at, 0));
        store(addr, addiu(MIPS_REG::k0, MIPS_REG::k0, -0x230));

        //Save every register
        for (int i = 0; i < 32; i++)
            store(addr, sq(i, MIPS_REG::k0, i * 16));

        //Store kernel stack pointer in SP
        store(addr, add(MIPS_REG::sp, MIPS_REG::k0, MIPS_REG::zero));

        //Load INTC_MASK and INTC_STAT and AND the result
        store(addr, lui(MIPS_REG::at, 0x10000000));
        store(addr, ori(MIPS_REG::t0, MIPS_REG::at, 0xF000));
        store(addr, lw(MIPS_REG::t1, MIPS_REG::t0, 0));
        store(addr, lw(MIPS_REG::t2, MIPS_REG::t0, 0x10));
        store(addr, and_ee(MIPS_REG::t2, MIPS_REG::t2, MIPS_REG::t1));

        //Clear INTC_STAT
        store(addr, sw(MIPS_REG::t2, MIPS_REG::t0, 0));

        //Load address from INTC handler and jump
        //TODO: loop through all handlers and check for cause
        store(addr, lui(MIPS_REG::at, 0x80000000));
        store(addr, ori(MIPS_REG::t0, MIPS_REG::at, 0xA000));
        store(addr, lw(MIPS_REG::t1, MIPS_REG::t0, 0));
        store(addr, jalr(MIPS_REG::ra, MIPS_REG::t1));
        store(addr, 0);

        //Restore location of saved registers
        store(addr, add(MIPS_REG::k0, MIPS_REG::sp, MIPS_REG::zero));

        //Load registers back
        for (int i = 0; i < 32; i++)
            store(addr, lq(i, MIPS_REG::k0, i * 16));
