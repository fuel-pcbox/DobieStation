...
namespace ipu
{
    ...
    ImageProcessingUnit::ImageProcessingUnit(ee::INTC* intc, ee::DMAC* dmac) : 
        intc(intc), dmac(dmac)
    {
    ...
    void ImageProcessingUnit::run()
    {
        ...
        if (can_write_FIFO())
            dmac->set_DMA_request(ee::DMAC_CHANNELS::IPU_TO);
        if (can_read_FIFO())
            dmac->set_DMA_request(ee::DMAC_CHANNELS::IPU_FROM);
    }
    ...
    void ImageProcessingUnit::finish_command()
    {
        ctrl.busy = false;
        command_decoding = false;
        intc->assert_IRQ((int)ee::Interrupt::IPU);
    }
    ...
    uint128_t ImageProcessingUnit::read_FIFO()
    {
        ...
        if (!out_FIFO.f.size())
            dmac->clear_DMA_request(ee::DMAC_CHANNELS::IPU_FROM);
        return quad;
    }
    ...
    void ImageProcessingUnit::write_FIFO(uint128_t quad)
    {
        ...
        if (in_FIFO.f.size() == 7)
        {
            dmac->clear_DMA_request(ee::DMAC_CHANNELS::IPU_TO);
        }
        if (in_FIFO.f.size() >= 8)
        {
