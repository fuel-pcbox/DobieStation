...
namespace gs
{
    GraphicsInterface::GraphicsInterface(GraphicsSynthesizer* gs, ee::DMAC* dmac) : 
        gs(gs), dmac(dmac)
    {
    ...
    {
        return !(path_queue & (1 << 3)) && active_path != 3;
    }
    ...
    void GraphicsInterface::request_PATH(int index, bool canInterruptPath3)
    {
        ...
        if (!active_path || active_path == index)
        {
            ...
            if (active_path == 3 && (!path3_masked(3) || FIFO.size() <= 15))
                dmac->set_DMA_request(ee::DMAC_CHANNELS::GIF);
    ...
    void GraphicsInterface::send_PATH3(uint128_t data)
    {
        ...
        else if (FIFO.size() < 16)
        {
            ...
            if (FIFO.size() > 15)
                dmac->clear_DMA_request(ee::DMAC_CHANNELS::GIF);
        }
    }
